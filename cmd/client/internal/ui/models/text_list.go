...
type (
	TextIDLoadedMsg struct {
		textIDs []int32
	}
)

func NewTextListModel(token, userID, login string, grpcClient *grpcclient.GophKeeperClient) TextListModel {
	m := TextListModel{
	...
	case tea.KeyMsg:
		...
		case "enter":
			if len(m.textIDs) > 0 && m.cursor < len(m.textIDs) {
				...
				detailModel := NewTextDetailModel(m.token, m.userID, m.login, m.grpcClient, credID)
...
func (m TextListModel) loadText() tea.Msg {
	...
	if err != nil {
		return TextLoadErrorMsg{Error: fmt.Sprintf("Ошибка загрузки: %v", err)}
	}
